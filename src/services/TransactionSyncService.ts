...
export class TransactionSyncService {
   ...
   * 从链上同步用户的所有交易
   */
  async syncTransactionsFromChain(userAddress: string, fromBlock: number = 0): Promise<void> {
    if (this.issyncing) {
    ...
    try {
      ...
      // 获取发送的交易
      const sentTxs = await this.getTransactionsByAddress(userAddress, 'from', fromBlock, currentBlock)
      
      // 获取接收的交易
      const receivedTxs = await this.getTransactionsByAddress(userAddress, 'to', fromBlock, currentBlock)
  ...
  async syncWithEtherscanAPI(userAddress: string): Promise<void> {
    try {
      ...
      if (normalResponse.status === '1') {
        const transactions = this.parseEtherscanTransactions(normalResponse.result, userAddress)
        await this.syncWithLocalStorage(transactions)
        console.log(`同步了 ${transactions.length} 笔普通交易`)
      }

      if (internalResponse.status === '1') {
        const internalTxs = this.parseEtherscanTransactions(internalResponse.result, userAddress)
   ...
   */
  private parseEtherscanTransactions(apiTxs: any[], userAddress: string): TransactionRecord[] {
   ...
   * 去重交易记录
   */
  private deduplicateTransactions(transactions: TransactionRecord[]): TransactionRecord[] {
   ...
   * 与本地存储同步
   */
  private async syncWithLocalStorage(chainTxs: TransactionRecord[]): Promise<void> {
    ...
    for (const chainTx of chainTxs) {
      ...
      } else if (localTx.status === 'pending' && chainTx.status !== 'pending') {
        // 更新待确认交易的状态
        await (transactionService as any).updateTransactionRecord(chainTx.hash, {
          status: chainTx.status,
          gasUsed: chainTx.gasUsed,
          blockNumber: chainTx.blockNumber
        })
        console.log(`更新交易状态: ${chainTx.hash} -> ${chainTx.status}`)
      }
    }
  }
  ...
  async incrementalSync(userAddress: string): Promise<void> {
    if (this.lastSyncBlock === 0) {
      ...
      // 增量同步
      await this.syncTransactionsFromChain(userAddress, this.lastSyncBlock + 1)
    }
  }
