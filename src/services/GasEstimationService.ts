...
import type { GasPriceData, TransactionRequest } from '../types/blockchain'
...
export class GasEstimationService {
	...
	async getTransactionGasInfo(transaction: TransactionRequest): Promise<{
		...
		try {
			...
			// 计算总费用（以Gwei为单位）
			const totalCosts = {
				slow: this.calculateTotalGasCost(
					gasLimit.toString(),
					gasPrices.slow
				),
				standard: this.calculateTotalGasCost(
					gasLimit.toString(),
					gasPrices.standard
				),
				fast: this.calculateTotalGasCost(
					gasLimit.toString(),
					gasPrices.fast
				)
			}
			...
			const totalCostsInETH = {
				slow: ethers.formatEther(
					ethers.parseUnits(totalCosts.slow, 'gwei')
				),
				standard: ethers.formatEther(
					ethers.parseUnits(totalCosts.standard, 'gwei')
				),
				fast: ethers.formatEther(
					ethers.parseUnits(totalCosts.fast, 'gwei')
				)
			}
