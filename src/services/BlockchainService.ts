...
import { NETWORKS, DEFAULT_NETWORK, RPC_CONFIG, GAS_CONFIG } from '../config/blockchain'
    ...
    try {
      ...
      return {
        slow: ethers.formatUnits(
          BigInt(Math.floor(Number(baseGasPrice) * gasPriceMultipliers.slow)), 
          'gwei'
        ),
        standard: ethers.formatUnits(baseGasPrice, 'gwei'),
        fast: ethers.formatUnits(
          BigInt(Math.floor(Number(baseGasPrice) * gasPriceMultipliers.fast)), 
          'gwei'
        ),
        estimatedTime: {
          slow: 300, // 5分钟
