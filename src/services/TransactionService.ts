...
export class TransactionService {
  ...
  async buildETHTransaction(
    ...
    try {
      ...
      if (gasPrice) {
        transaction.gasPrice = ethers.parseUnits(gasPrice, 'gwei').toString()
      }
   ...
   * 获取单个交易记录
   */
  async getTransactionRecord(txHash: string): Promise<TransactionRecord | null> {
    try {
      const records = await this.getTransactionHistory()
  ...
  async getTransactionHistory(address?: string): Promise<TransactionRecord[]> {
    try {
      const records = await StorageService.getItem<TransactionRecord[]>('transaction_history') || []
