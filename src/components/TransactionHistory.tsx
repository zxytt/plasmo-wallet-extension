...
function TransactionItem({ transaction, currentAddress, onViewDetails }: TransactionItemProps) {
  const isOutgoing = transaction.from.toLowerCase() === currentAddress.toLowerCase()
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="text-right ml-4">
          <div className={`font-bold text-lg mb-1 ${isOutgoing ? 'text-red-600' : 'text-green-600'}`}>
            {isOutgoing ? '-' : '+'}{formattedAmount}
          </div>
          <div className="text-xs text-gray-500 mb-2">ETH</div>
          <div className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${statusDisplay.bgColor} ${statusDisplay.color} border`}>
...
export function TransactionHistory({ currentAddress, onRefresh }: TransactionHistoryProps) {
  ...
  const loadTransactionHistory = async () => {
    try {
      ...
      const history = await transactionService.getTransactionHistory(currentAddress)
  ...
  const checkPendingTransactions = async () => {
    try {
      ...
      for (const tx of pendingTxs) {
        const updatedRecord = await transactionService.trackTransaction(tx.hash)
        if (updatedRecord && updatedRecord.status !== 'pending') {
          // 更新本地状态
          setTransactions(prev => 
            prev.map(t => t.hash === tx.hash ? updatedRecord : t)
          )
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex space-x-3">
          <Button onClick={loadTransactionHistory} variant="secondary" className="flex-1">
            重试
          </Button>
        </div>
      </div>
  ...
  if (selectedTransaction) {
    const isOutgoing = selectedTransaction.from.toLowerCase() === currentAddress.toLowerCase()
    const statusDisplay = (() => {
      switch (selectedTransaction.status) {
        case 'pending':
          return { icon: '⏳', text: '待确认', color: 'text-yellow-600', bgColor: 'bg-yellow-50', borderColor: 'border-yellow-200' }
        case 'success':
          return { icon: '✅', text: '交易成功', color: 'text-green-600', bgColor: 'bg-green-50', borderColor: 'border-green-200' }
        case 'failed':
          return { icon: '❌', text: '交易失败', color: 'text-red-600', bgColor: 'bg-red-50', borderColor: 'border-red-200' }
        default:
          return { icon: '❓', text: '未知状态', color: 'text-gray-600', bgColor: 'bg-gray-50', borderColor: 'border-gray-200' }
      }
    })()
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div className="px-6 py-4 bg-gray-50 border-b">
            <div className="text-center">
              <div className="text-sm text-gray-600 mb-1">交易金额</div>
              <div className={`text-2xl font-bold ${isOutgoing ? 'text-red-600' : 'text-green-600'}`}>
                {isOutgoing ? '-' : '+'}{formatAmount(selectedTransaction.value)} ETH
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-4 space-y-4">
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  <div className="text-sm text-gray-600 mb-1">发送方</div>
                ...
                <div>
                  <div className="text-sm text-gray-600 mb-1">接收方</div>
                  <div className="text-gray-800 font-mono text-sm bg-gray-50 p-2 rounded">
              ...
              {selectedTransaction.gasUsed !== '0' && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-sm text-gray-600 mb-1">Gas 使用</div>
                    <div className="text-gray-800 font-mono text-sm bg-gray-50 p-2 rounded">
                      {parseInt(selectedTransaction.gasUsed).toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600 mb-1">Gas 价格</div>
                    <div className="text-gray-800 font-mono text-sm bg-gray-50 p-2 rounded">
              ...
              {selectedTransaction.blockNumber && (
                <div>
                  <div className="text-sm text-gray-600 mb-1">区块号</div>
                  <div className="text-gray-800 font-mono text-sm bg-gray-50 p-2 rounded">
              ...
              <div>
                <div className="text-sm text-gray-600 mb-1">交易时间</div>
                <div className="text-gray-800 text-sm bg-gray-50 p-2 rounded">
                  {new Date(selectedTransaction.timestamp).toLocaleString('zh-CN', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit',
                    hour: '2-digit',
          ...
          <div className="px-6 py-4 bg-gray-50 border-t">
            <Button
              onClick={() => handleViewInExplorer(selectedTransaction.hash)}
              variant="primary"
              className="w-full"
            >
              🔍 在 Etherscan 中查看
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {transactions.length > 0 ? `共 ${transactions.length} 笔交易` : '暂无交易记录'}
          </p>
        </div>
      ...
      {transactions.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            您还没有进行过任何交易。发送或接收 ETH 后，交易记录将显示在这里。
          </p>
        </div>
      ) : (
        <div className="space-y-3">
