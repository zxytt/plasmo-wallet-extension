...
export function PasswordStrengthIndicator({ strength, password }: PasswordStrengthIndicatorProps) {
  ...
  return (
    <div className={`p-4 rounded-lg border ${getStrengthBg(strength.score)}`}>
      {/* 强度条和标签 */}
      ...
      <div className="space-y-1">
        <div className="text-xs font-medium text-gray-600 mb-2">密码要求:</div>
        <div className="grid grid-cols-1 gap-1 text-xs">
          ...
          <div className={`flex items-center space-x-2 ${strength.requirements.hasLower ? 'text-green-600' : 'text-gray-500'}`}>
            <span>{strength.requirements.hasLower ? '✅' : '⭕'}</span>
            <span>包含小写字母</span>
          </div>
          <div className={`flex items-center space-x-2 ${strength.requirements.hasUpper ? 'text-green-600' : 'text-gray-500'}`}>
            <span>{strength.requirements.hasUpper ? '✅' : '⭕'}</span>
            <span>包含大写字母</span>
          </div>
          <div className={`flex items-center space-x-2 ${strength.requirements.hasNumber ? 'text-green-600' : 'text-gray-500'}`}>
            <span>{strength.requirements.hasNumber ? '✅' : '⭕'}</span>
            <span>包含数字</span>
          </div>
          <div className={`flex items-center space-x-2 ${strength.requirements.hasSpecial ? 'text-green-600' : 'text-gray-500'}`}>
            <span>{strength.requirements.hasSpecial ? '✅' : '⭕'}</span>
            <span>包含特殊字符 (推荐)</span>
          </div>
        </div>
      </div>
