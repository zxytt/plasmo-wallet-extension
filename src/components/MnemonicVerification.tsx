...
export function MnemonicVerification({
	...
	const getInputClassName = (index: number) => {
		...
		const baseClasses =
			'w-full p-3 text-center border rounded-lg text-sm font-medium'
	...
	const canVerify = wordsToVerify.every(
		(index) => inputWords[index].trim() !== ''
	)
			...
			<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
				<div className="flex items-start space-x-2">
					...
					<div className="text-blue-800 text-sm">
						...
						<div>
							请在高亮的输入框中输入对应位置的助记词单词，灰色区域无需填写
						</div>
					</div>
				</div>
			</div>
					...
					{Array.from({ length: 12 }, (_, index) => (
						<div key={index} className="space-y-1">
							...
							<input
								...
								onChange={(e) =>
									handleWordInput(index, e.target.value)
								}
